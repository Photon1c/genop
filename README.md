...
[Now live](https://generativeoptogenetics.netlify.app/)
...
# Or drag & drop the genop folder to https://app.netlify.com/drop
```

The dashboard shows:
- **Live expression visualization**: Color-coded petri dish (red = low expression, green = high)
- **Real-time metrics**: Time step, MSE, energy, toxicity, reward, sensitivity
- **Interactive controls**: Sliders for all parameters (desensitization, dynamics, pulse controller)
- **Multiple controllers**: Greedy, Random, Pulse
- **CMA-ES optimization**: Browser-based parameter optimization
- **Desensitization enabled**: Watch sensitivity decrease with continuous activation
...
- **`test_script2.py`**: Desensitization + latent controller + CMA-ES (Python)
- **`dashboard.html`**: Interactive web-based visualization (Three.js, standalone)
- **`app.py`**: Flask app for serving dashboard and API endpoints
